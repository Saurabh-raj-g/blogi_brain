...
export default class UserUseCase {
    ...
    public async create(
        ...
        try {
            ...
            saved!.emailVerificationExpire = DateTime.now().plus(
                Env.get("EMAIL_VERIFICATION_TOKEN_EXPIRE") * 60 * 1000
            );
    ...
    public async verifyEmail(userId: string, token: string): Promise<void> {
        ...
        const verified = await TokenVerificationService.verify(
            userEntity.emailVerificationToken!,
            token,
            userEntity.emailVerificationExpire!
        );
    ...
    public async changeEmailRequest(
        ...
        try {
            ...
            userEntity.emailVerificationExpire = DateTime.now().plus(
                Env.get("EMAIL_VERIFICATION_TOKEN_EXPIRE") * 60 * 1000
            );
            await this.userRepository.save(userEntity);
        } catch (e) {
        ...
        try {
            await NotificationService.send(
                NotificationEvent.changeEmailRequest(),
                options
            );
        } catch (e) {
            userEntity.emailVerificationToken = null;
    ...
    public async updateAvatar(
        ...
        try {
            ...
            const myCloud = await cloudinary.upload(file, file.clientName, {
    ...
    public async sendResetPasswordToken(
        ...
        try {
            ...
            userEntity.resetPasswordExpire = DateTime.now().plus(
                Env.get("RESET_PASSWORD_TOKEN_EXPIRE") * 60 * 1000
            );
            await this.userRepository.save(userEntity);
        } catch (e) {
    ...
    public async resetPassword(
        ...
        const verified = await TokenVerificationService.verify(
            ...
            token,
            userEntity.resetPasswordExpire!
        );
